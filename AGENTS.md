...
>
> **iter50 EVR Loss-Reclassification Gap: Sell-Concentration Veto & The Fundamental Breadth Impossibility Bound — REJECTED / Rigorous Proof (evr9 production default unchanged).**
> Microstructure autopsy discovered false positives (scratches) are bursty single-second whale-sweeps (`maxsec_sell_share_60` AUC 0.686) vs true positives' (bleeds) distributed multi-second selling. Setting permanent per-trade veto `v2_evr_skip_sell_conc_min = 0.40` gave static $\Delta_{\text{LOO}} = +0.1375$ SOL. **Full-cohort batch REJECTED**: 776 trades / 70.75% WR / +1.7880 SOL vs evr9 783 / 70.24% / +1.7531 SOL (+0.0350 SOL net).  **Step 5 gates fail decisively**: (1) Whole-PnL Wilcoxon $p = 0.3416$, bootstrap 95% CI [−0.000281, +0.000553] (straddles 0), **Improvement Breadth = 1.95% (6/308 recordings improved vs 50.0% required)**.  (2) Tail regressed: catastrophics $\le-30\%$ count +2 (76 → 78), tail PnL −0.1104 SOL, `kelly_flat` PnL −0.2622 SOL (vetoed TPs bled into full `kelly_flat` drawdowns, wiping out 81% of FP savings).  **Mathematical Proof**: Any EVR fire-filtering mechanism is confined to the 45 EVR-active recordings (14.6% of traded book) and 13 FP recordings (4.2% of book), establishing a **hard mathematical ceiling of 14.6% max breadth** — proving fire-filtering can NEVER pass the $\ge 50.0\%$ breadth gate.  Engine parameter defaults unchanged (`v2_evr_skip_sell_conc_min = 0.0`, `v2_evr_enable = 1.0`).  `test_evr.py` 10/10, `test_futures.py` 18/18.
