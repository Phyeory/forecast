...
10. **Whale-Dump Confirmed Exit (`whale_dump_exit`, iter72 — re-implemented at iter78 from the unit-test spec after the original code was lost in the working tree; **DEFAULT OFF (`v2_whale_dump_exit_enable=0.0`) pending user re-adoption**; iter78 re-gate on the grown 2,127-rec DB: spec cell (min_usd 200) 6 fires / Δ−0.024 / era-inverted (OLD +0.008 / DEAD −0.032) / tail unchanged at every band → REJECTED; min_usd 100 cell 3 fires / Δ−0.007 / inert → REJECTED. The iter72 adoption record is preserved below for provenance; the mechanism ships parity-safe (bare-{} byte-equal proven vs the fresh baseline; `analysis/test_whale_dump.py` 10/10 under the iter78 default-OFF contract). See RESEARCH_LOG.md Iter 72 + Iter 78 §4.)**: a candle whose sell volume ≥ `v2_whale_dump_min_usd` ($200; USD via market_cap/(close×1e9) — no external feed, no holder_flow dependency, ~100% candle coverage, ≤1 s arming + 5-candle ≈6 s worst-case print-to-fire) landing on a never-armed (peak ≤ `v2_whale_dump_max_peak_pct` = +5%) trade already ≥ `v2_whale_dump_offside_pct` = 8% offside at the print close, whose price STAYS ≤ print-close × (1 − `v2_whale_dump_confirm_g`/100) for `v2_whale_dump_confirm_s` = 5 **distinct candles** (the 5-candle persistence is the causal dump/absorbed-sweep discriminator — every un-confirmed print-exit variant is class-(D) net-negative; print-time features are class-(A) at full coverage). Measured before the iter72 adoption: dense +0.0175 SOL / full-953 +0.0514 SOL, tail counts UNCHANGED at every band, tail PnL improved at every band, kelly_flat +0.047..+0.123, zero winner cuts post-fix, replacement entries positive, 2-3 fires/cohort (statistically unproven — iter57-style override, not a battery acceptance). Escape hatch: `{"v2_whale_dump_exit_enable": 1.0}` re-arms; 0.0/default = byte-parity with B′. Do NOT weaken the confirmation (K<5 or g<3) — the v1 sub-tick bug measured exactly that regime and it adds soft tail. See RESEARCH_LOG.md Iter 72.
