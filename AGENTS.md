...
> **iter58 Live-vs-Backtest Pipeline Parity Fix — three root causes eliminated, no strategy change (engine byte-identical).**
> Live sessions diverged from backtests of their own auto-recordings (backtester: fewer trades, higher WR/PnL; each side produced entries the other never would). Root causes + fixes, all in `live_trader.py`/`main.py` (`strategy_engineV2.py` untouched):
> (1) **Entry-reference price mismatch (dominant).** The backtester notifies the engine at its fill price — for a signal at intra-candle state $k<4$ that is state $k{+}1$ of the SAME candle (mid-candle fill via `_intrabar_price` × `_fill_fraction()`), for $k=4$ the next candle's open — always × slippage. Live notified the signal-candle close. Every V2 exit threshold references `engine.entry_price`, so a ~1–2% skew chaotically flipped exit reasons/re-entry windows. Live now reconstructs the exact backtest fill price (deterministic from the completed candle) and notifies with it; `main.py` passes `boundary_open_price` (incoming trade price = next candle's recorded open) for state-4 signals.
> (2) **Pending signals silently dropped.** `_process_completed_candle` cleared `_pending_buy/_exit` unconditionally every boundary — any signal whose swap was blocked (another swap in flight ~1–6 s, buy TX settling ≤75 s) was lost forever, so live held positions the backtest had exited. Signals are now ARMED UNTIL EXECUTED with retry at Step 0 of every boundary (`_try_execute_pending`); exits held while buy TX unresolved, deduped vs already-dispatched sells (the main.py holder-flow immediate-exit now also sets `status="closing"`).
> (3) **Stale candle buffer.** `main.py` skipped `live_trader.update()` on unchanged-price ticks, so the final same-price trade's volume/buy-sell split never reached the engine — live evolved on different candle data than the stored recording. `update()` now runs on EVERY tick; only the WS broadcast is throttled.
> Additionally the expansion now interleaves engine notifications at the exact backtest fill point (mid-candle exits close the position between sub-states, enabling same-candle re-entry arming exactly like `ft.update()`'s per-state Step 1); historical warmup passes `engine_exec=False` and stays inert.
> **Validation:** lockstep harness `backend/analysis/batch_parity.py` — 60/60 recordings ENGINE-IDENTICAL across every candle (RBPF particle means bit-equal); trade sequences identical to the second on recs {2750, 2899, 2933, 2935}; regression suite `backend/test_live_parity.py` 8/8; `test_futures.py` 18/18 (spot parity). Residual gaps are execution reality only (swap latency, buy-failure rollback + 120 s re-entry block, mcap-floor sells) — explicitly out of scope.
