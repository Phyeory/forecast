...
- **Entry gate** (`strategy_engineV2.py`): Blocks entry if a dev/insider sell occurred in the last 30 s. **Default OFF** (`v2_holder_flow_entry_block=0.0`) since iter38 — data collection runs regardless; the gate is re-enabled only for A/B comparison.
- **Exit trigger** (`strategy_engineV2.py` + `forward_tester.py`): Fires an immediate exit if a dev/insider sell occurs while in position. **Default OFF** (`v2_holder_flow_exit_enable=0.0`) since iter38.
- **`v2_holder_flow_require_tag`** (default 1.0): when > 0 the gate/exit only fire on *verified* insider tags (`_DEV_TAGS`); the `whale` fallback and untagged events do NOT qualify. Set to 0.0 to reproduce the iter36/38 legacy "any big seller" circuit-breaker. This is the knob that distinguishes **gate 2.0** (require_tag=1) from **gate 1.0** (require_tag=0) in the planned A/B/C comparison.
...
The mechanism is parity-safe when no `holder_flow` data exists (the gates never fire on an empty table, so legacy recordings are byte-identical). With the iter38 defaults (gate/exit OFF), behaviour is byte-identical to the pre-iter36 engine on every recording, with or without events.
