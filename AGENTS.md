...
10. **Whale-Dump Confirmed Exit (`whale_dump_exit`, iter72 — PRODUCTION ON since 2026-08-30 by explicit user decision; battery verdict on record: REJECTED on power, net-positive with zero added tail on both cohorts)**: a candle whose sell volume ≥ `v2_whale_dump_min_usd` ($200; USD via market_cap/(close×1e9) — no external feed, no holder_flow dependency, ~100% candle coverage, ≤1 s arming + 5-candle ≈6 s worst-case print-to-fire) landing on a never-armed (peak ≤ `v2_whale_dump_max_peak_pct` = +5%) trade already ≥ `v2_whale_dump_offside_pct` = 8% offside at the print close, whose price STAYS ≤ print-close × (1 − `v2_whale_dump_confirm_g`/100) for `v2_whale_dump_confirm_s` = 5 **distinct candles** (the 5-candle persistence is the causal dump/absorbed-sweep discriminator — every un-confirmed print-exit variant is class-(D) net-negative; print-time features are class-(A) at full coverage). Measured before adoption: dense +0.0175 SOL / full-953 +0.0514 SOL, tail counts UNCHANGED at every band, tail PnL improved at every band, kelly_flat +0.047..+0.123, zero winner cuts post-fix, replacement entries positive, 2-3 fires/cohort (statistically unproven — iter57-style override, not a battery acceptance). Adoption verified both directions 5/5: bare `{}` reproduces the adopted batches; `{"v2_whale_dump_exit_enable": 0.0}` restores pre-iter72 byte-parity (the escape hatch). Do NOT weaken the confirmation (K<5 or g<3) — the v1 sub-tick bug measured exactly that regime and it adds soft tail. Re-run the formal battery when ≥30 live/post-wiring-fix fires accumulate; if live shows any added soft tail, flip the explicit 0.0. See RESEARCH_LOG.md Iter 72.
