...
6. **Pre-iter15 recordings have zero order flow**: Any backtest run against existing `backend/data/price_data.db` recordings (iter01–iter14 baselines) carries `buy_volume = sell_volume = 0` everywhere and `volume > 0` on only 0.07% of candles. The iter15 `PumpSwapRPCClient` recorder patch (`backend/pumpfun_client.py`) fixes future recordings only; existing recordings cannot be retro-fixed because the per-trade vault deltas were never persisted. Do not run engine experiments against the legacy dataset without acknowledging in the writeup that the engine is structurally input-starved; re-record fresh data first.
