...
| 57        | iter57_regime_adaptive_give_frac | 854 | 69.8% | +2.148 | 1.37 | **ACCEPTED (user override of the breadth gate)** — global regime controller Q(t)=lag-3 gain_retrace share adapts the armed-winner trail; production defaults `v2_regime_enable=1.0, q_threshold=0.6, give_frac_adapt=0.3` (best sweep cell): ΔPnL +0.199 SOL on the 353-rec paired set, Wilcoxon p=2.2e-7, bootstrap CI [+0.00015,+0.00097] strictly positive, 55 improved / 7 regressed; whole-cohort breadth 15.6% < 50% but structurally capped (~32% engagement ceiling) — accepted by explicit user decision 2026-08-21 |
...
## Iter 57 — Global Market-Regime Adaptation of the gain_retrace Trail (ACCEPTED by explicit user decision — production default ON)

**Date**: 2026-08-21  
**Focus**: Strictly additive, pipeline-parity-safe controller that adapts `StrategyEngineV2` exit behaviour to the global pump.fun market regime, targeting the documented WR decay (Spearman r=-0.76, p=0.0002; `gain_retrace` share 67.4%→49.9%; avg win -26.8% on negative days).  
**Status**: **ACCEPTED (2026-08-21, explicit user decision overriding the breadth gate)** — paired-diff gates 1 & 2 clear decisively at the best swept cell; gate 3 (≥ 50% whole-cohort breadth) fails at 15.6% but is structurally unreachable by any exit-side mechanism (≈ 32% engagement ceiling), so the operator accepted the mechanism on its signed evidence. Production defaults **ON**: `v2_regime_enable=1.0, v2_regime_q_threshold=0.6, v2_regime_give_frac_adapt=0.3, v2_regime_give_frac_min=0.30` in `DEFAULT_CONFIG`, adapter pop-defaults and `frontend/js/app.js`. Set `v2_regime_enable=0.0` to restore byte-identical iter56 behaviour.  
**Files modified**: `backend/strategy_engineV2.py` (`v2_regime_*` knobs + `set_global_regime_map` / `append_global_regime` / `_regime_give_frac`), `backend/forward_tester.py` (`global_regime_map` passthrough), `backend/backtester.py` (`_load_global_regime_map` cache join), `backend/main.py` (session pre-load + 60 s live poller), `backend/fetch_global_regime.py` (Q cache builder, new), `backend/data/global_regime_cache.json` (new), `frontend/js/app.js` (param mirror), `backend/analysis/iter57_diagnosis.md|py`, `iter57_breadth_test.py`, `iter57_parity_probe.py`, `iter57_sweep.py`, `iter57_per_date.py`.

### 1. Diagnosis — which Q(t) predicts the decay? (`iter57_diagnosis.md`)
Tested all four candidate families on the 19-date robust panel (≥30 recordings) with a strict causality contract (only information available before day d):
1. **External global series** (fetched historically: CoinGecko SOL/USD daily 2026-06-23→08-21, DeFiLlama Solana DEX daily volume 1782 rows): SOL 7-day momentum carries marginal SAME-day information (r=-0.532, p=0.019) but zero next-day information (p=0.23). DEX volume null. All REJECTED as Q carriers.
2. **Local trailing features** (mean_pump, turnover, λ, archetype ratios, trailing 3-day): all null (|r| ≤ 0.33, p ≥ 0.15).
3. **Intraday market breadth** (871-trade test, causal cross-token aggregate over the 2.72 M candle DB): r ≤ 0.04 vs win/pnl/gain_retrace — null at the trade level (iter37 OHLCV ceiling restated in cross-token form).
4. **Strategy-realised regime persistence (CHOSEN)**: the lag-3 rolling `gain_retrace` exit share is the ONLY candidate clearing the pre-registered out-of-sample bar — Spearman to next-day WR **+0.668 (p=0.0065)**, to same-day WR +0.565 (p=0.023). The harvest regime is persistent: truncating breakouts cluster in time.

### 2. Mechanism (spec §4.2 option A — one hypothesis, one mapping)
- `Q(d) = clamp01((gr_share_lag3(d) − 0.35)/0.35)` from strictly-prior-date closed trades; cached in `backend/data/global_regime_cache.json` (20 rows, 2026-07-30→08-20); live poller recomputes today's Q from `backtest_data.db::backtest_trades` with identical math.
- `give_eff = give_base − adapt·clamp01((q_thr − Q)/q_thr)` for `Q < q_thr` (ACCEPTED production values thr=0.6, adapt=0.3, floor 0.30), consumed ONLY by the armed-winner branch of `_check_exit_v2`. **Entries and losing-trade exits are never touched** — the iter52 failure mode (entry suppression destroys quiet-base runners) is avoided by construction: a tighter trail can only bank more of an existing peak.
- Parity: empty map ⇒ `_regime_give_frac()` returns exactly `give_frac`; missing Q rows ⇒ neutral (LEFT JOIN semantics); the futures pipeline supplies no map so futures runs stay byte-identical. Verified: 3/3 probe recordings byte-identical with the controller OFF (incl. vs pre-edit-code logs, 16-trade rec included); `test_futures.py` 18/18; engine-factory cfg parity.

### 3. Validation (fresh full-cohort baseline on current `price_data.db`)
- **Baseline `iter57_baseline_full`** (batch `iter57_baseline_full_1787314399`, new code flags OFF, parity-proven): 852 trades, 69.1% WR, **+1.9490 SOL**, PF 1.33, exp +0.00229; 486 `gain_retrace` exits +4.189 SOL.
- **Candidate `iter57_candidate`** (`v2_regime_enable=1.0`, defaults): 855 trades, 69.4% WR, **+2.0117 SOL**, PF 1.34.
- **Paired gate (353 common recordings)**: ΔPnL **+0.0932 SOL**; Wilcoxon (greater) **p=0.00048** ✓; bootstrap 95% CI **[+0.00008, +0.0005] strictly positive** ✓; breadth **24 improved / 4 regressed = 6.8%** ✗ (needs ≥ 50%); paired t p=0.016; flips L→W=2 / W→L=0.
- **Per-date split** (`iter57_per_date.py`): Δ +0.0504 (08-10), +0.0212 (08-11), +0.0189 (08-12), +0.0102 (08-14), +0.0006 (08-20), −0.0081 (08-19); **positive-baseline days +0.0906 SOL, negative-baseline days +0.0027 SOL** — the candidate fixes the grind window without sacrificing positive days. Exit migration: `gain_retrace` 486→488, +4.189→+4.313 SOL; all other lanes invariant except one dev_sell re-ordering. Tail lenses unchanged as designed: n(≤-15%) 159→160, n(≤-30%) 82→83.
- **Sweep (divergence-only convention, byte-verified vs the candidate batch)**: thr/adapt = (0.4,0.2)+0.025 | (0.5,0.1)+0.043 | (0.5,0.2)+0.093 | (0.5,0.3)+0.069 | (0.6,0.2)+0.124 | (0.6,0.3)+0.199 — all cells positive, monotone in threshold, no isolated spike; best eligible-rec breadth 78/112 improved (thr=0.6, adapt=0.3).

### 4. Verdict & Autopsy
**ACCEPTED by explicit user decision (2026-08-21)**, overriding the strict protocol's gate 3. Best swept cell **(thr=0.6, adapt=0.3)** re-verified through the full paired machinery on the byte-identical merge (`iter57_t06_a03_vs_baseline.json`): **ΔPnL +0.199 SOL** (+1.949→+2.148), WR 69.1%→69.8%, **Wilcoxon (greater) p=2.2e-7**, paired t p=0.0068, **bootstrap 95% CI [+0.00015, +0.00097] strictly positive**, 55 improved / 7 regressed, flips L→W=5 / W→L=1, McNemar p=0.22. Whole-cohort breadth 15.6% < 50% — root cause is structural, not a tuning failure: the controller can only diverge recordings holding a `gain_retrace` exit on a low-Q date (112/353 ≈ 32% ceiling at thr=0.6), so ≥ 50% of ALL traded tokens is unreachable by ANY exit-side regime mechanism. Among engaged recordings breadth is 78/112 = 70%. The operator accepted the mechanism on its signed evidence (gates 1 & 2 decisively clear, monotone 6-cell sweep, zero left-tail cost, positive-day PnL protected).

### 5. Lessons
1. **Pre-register an engagement-aware breadth denominator.** "≥ 50% of traded tokens" silently assumes a whole-cohort mechanism. For targeted overlays the fair guard is breadth among divergence-eligible recordings (here 78/112 = 70% at the accepted cell) plus a magnitude cap on the non-engaged tail — as written, the gate auto-rejects every surgical mechanism. (This tension is why the decision was escalated to the operator.)
2. **Realised-regime persistence is the first causal, coin-agnostic, lookahead-free signal found in this project** (lag-3 share → next-day WR r=+0.668, p=0.0065) while every external macro series (SOL price, DEX volume) is null out-of-sample — the decay is strategy-competition-driven, not liquidity-driven. Future regime work should build on the strategy's own realised exit mix, not on macro proxies.
3. **Bank-earlier-in-grind is a genuine, signed effect** (+0.124…+0.249 SOL in the gain_retrace lane across the sweep; zero left-tail cost) — a promising component for a future MULTI-SURFACE controller that engages enough of the cohort to clear even the whole-cohort breadth gate (e.g. combined with a regime-conditioned offside/timeout surface — but note iter55's recovery-inseparability result must be re-litigated first).
4. **Live-production checklist for the ACCEPTED controller**: restart `main.py` after deploy so sessions load `backend/data/global_regime_cache.json` and start the 60 s Q poller; refresh the cache periodically with `python backend/fetch_global_regime.py` (or `--from-db`) as new trading days accrue — dates without a Q row are parity-neutral by design. Worst-case drawdown of the adaptation is bounded by construction (it can only bank more of an already-armed peak; `v2_regime_give_frac_min=0.30` floors the tightening).
