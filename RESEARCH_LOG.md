...
### 7. Post-adoption dead-code cleanup (user decision 2026-09-02): every enabled mechanism verified at its adopted battery; all never-to-be-enabled mechanisms removed — decisions byte-identical

**Verification BEFORE surgery:** each production mechanism's live parameters were re-read off the engine object and matched against its adoption battery — EVR evr9 + iter50 veto (0.25/60), rate_split ungated (θ0.55/K12/arm10), holder-flow gates (1.0/1.0, windows 30/15 s, $100, require_tag 0), MSM B′ (`0:idle;2:idle,trend`), gain_retrace (10/0.5), kelly_flat (60/40%), breakeven (25/2.5), warmup 400 intakes, `v2_entry_delay_seconds=5.0`. ALL MATCH. A pre-surgery snapshot (bare {} on recs {2762, 1019, 3814, 2119}) was locked and verified byte-equal to the adopted `iter78_lat5` batch.

**Removed from the engine (689 lines):** futures second param-set (preset/overrides/`_is_futures_engine`/fut knobs + update-path branches — the futures layer was deleted from the tree 2026-08-29, these were orphans); whale-dump confirmed exit (§4, both cells rejected); iter75 s2 gate + fleet-medium machinery; iter56/68 hf-silence gate (rejected at the warmup-400 re-gate); iter45 OFI gate (full-batch-rejected); iter05 leading-decay EXIT branch + persist/offside knobs + `s_effective_min` gate — **the entry-side decay block is behaviorally ON (default 1.0) and was KEPT, with its window/thresh knobs**; iter64 Q-gate machinery (cache loader, live map setter, `_regime_q_today`, `_rate_split_regime_allows`, regime_gate/q_max/unknown_q knobs — the flip is ungated); `v2_require_past_peak` (iter17b); unused `mu_exit_*` trio; `v2_msm_occupancy_floor` (iter74b; `state_at(occupancy_floor=0.0)` fixed).

**Removed consumers:** `backtester.py` `_MediumSource` + s2 injection; `main.py` iter57 regime-cache maintenance loop, fleet-vov medium push, `_global_regime_pump`; `backend/fetch_global_regime.py` deleted. **Kept:** V1-era attr pops (captured in per-trade `cfg_*` snapshots), iter05 core trackers (live entry block consumes them), `_candle_volume_history` (EVR windows).
