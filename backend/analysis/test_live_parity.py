...
def _feed_ft(candles, engine_kwargs=None):
    """Live-parity semantics: ForwardTester over the candle list with
    run_backtest's default slippage (1.0 %) — the reference configuration.

    iter78 adoption: mirror run_backtest's engine-keyed deferred-entry
    injection — when the ENGINE's `v2_entry_delay_seconds` knob is > 0
    (the adopted 5.0 s default), switch the tester into latency mode so
    the backtest reference fills entries at t_signal+N on the recorded
    path, exactly like production backtests.  Explicit
    `{"v2_entry_delay_seconds": 0.0}` restores the pre-iter78 instant
    reference.
...
def test_decision_parity_with_backtester(rec):
    ...
    # iter61 note (historical): the participation-floor pin was removed with
    # the floor itself (2026-08-24, iter64 surgery).  The iter63/64 rate-split
    # mechanism is additionally regime-gated by the causal Q(today); to keep
    # decision-parity date-independent, explicitly DISABLE rate-split here.
    # iter86b note: the per-coin online recalibration (adopted 2026-09-13)
    # is likewise pinned OFF here — this harness constructs engines DIRECTLY
    # (bypassing the pipeline calibration layers), and its contract is the
    # engine mechanics (signal → decision parity), not the calibration
    # stack's session-tape adaptation.  The recalibrate() primitive's own
    # parity is engine-native by construction (rbpf.step consumes cfg per
    # tick); its semantics are pinned by test_percoin_calibration.py.
    parity_kwargs = {"v2_rate_split_enable": 0.0,
                     "v2_percoin_cal_enable": 0.0}

    # ── Contract A (escape hatch): delay=0 → EXACT decision parity ──────
    # The iter73 signal-instant contract.  `v2_entry_delay_seconds=0.0` and
    # `v2_exit_delay_seconds=0.0` (the iter80 escape hatch) restore it
    # byte-exactly — pinned here so the adoptions' escape hatches can never
    # silently drift.
    parity_kwargs = {"v2_rate_split_enable": 0.0,
                     "v2_percoin_cal_enable": 0.0,
    ...
    ft0 = _feed_ft(candles, engine_kwargs=dict(parity_kwargs))
    trader0 = _make_trader(engine_kwargs=dict(parity_kwargs))

    async def drive0():
        await _feed_live(trader0, candles)
        await asyncio.sleep(0)
    asyncio.run(drive0())

    bt0 = [(t.entry_time, t.entry_reason) for t in ft0.trade_history]
    lv0 = [(t.entry_time, t.entry_reason) for t in trader0.trade_history]
    assert len(lv0) == len(bt0), (
        f"instant parity trade-count mismatch: live {lv0} vs bt {bt0}")
    for (lt, bt) in zip(lv0, bt0):
        assert abs(lt[0] - bt[0]) <= 1, f"instant entry time mismatch: {lt} vs {bt}"
        assert lt[1] == bt[1], f"instant entry reason mismatch: {lt} vs {bt}"

    # ── Contract B (adopted default): delay=5 → WINDOW parity ───────────
    # The iter78 adopted deferred-entry cell.  Both sides defer entry fills
    # by the engine's `v2_entry_delay_seconds` (5 s default) — the live path
    # fires on the first executor pass at/after t_signal+N, the backtest
    # resolves the fill on the recorded path in the same window.  This
    # harness feeds ONE tick per candle (the boundary-batched 4-state
    # expansion), so the engine's signal candle can differ by one state
    # from the backtest's streaming 4-state feed after a mid-recording
    # exit — exact state-level parity is pinned by Contract A; under the
    # adopted cell the pinned guarantees are trade COUNT and the entry
    # WINDOW (delay + 1 state).
    ft = _feed_ft(candles, engine_kwargs=dict(parity_kwargs))
    ...
    trader = _make_trader(engine_kwargs=parity_kwargs)
    ...
    assert len(live_tr) == len(bt_entry), (
        ...
        f"live={live_tr}\nbt={bt_entry}")
    _tol = 1 + int(float(getattr(trader, "entry_delay_seconds", 0.0)))
    for (lt, bt) in zip(live_tr, bt_entry):
        assert abs(lt[0] - bt[0]) <= _tol, f"entry time mismatch: {lt} vs {bt}"
        assert lt[1] == bt[1], f"entry reason mismatch: {lt} vs {bt}"
    # exit reasons: compared on the instant contract above; under the
    # deferred cell an exit can shift across an age-clock boundary
    # (EVR 120 s) from a one-state entry offset — recorded, not asserted.
