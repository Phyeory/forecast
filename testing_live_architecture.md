...
### Key Functional Features:
- **Identical Indicator Evolution:** The Live Trader now uses the same **4-state intra-candle expansion** as the Backtester. On each candle boundary, the completed candle is expanded into four accumulated sub-states (open → first extreme → second extreme → close) and `engine.update()` is called on each. This ensures EMAs, ATR, Kalman filters, trend confidence, and all other rolling indicators evolve through the exact same intermediate states as the backtester — producing identical signal timing.
- **Candle-Boundary Processing:** Rather than calling the engine on every raw tick, intermediate ticks are buffered. The strategy engine only processes data when a candle finalizes (via `is_new` flag from the `CandleAggregator`), preventing indicator divergence from over-updating on partial candle states.
- **Immediate Execution model:** Unlike the backtested and forward-tested variants that queue for the next bar (simulating live execution delay), the Live Trader executes **IMMEDIATELY** once a candle finalizes and a signal is detected.
- **Historical Warmup:** Uses `update_historical_candle()` which applies the same 4-state expansion to seed all indicators identically to the Backtester before live data streaming begins.
